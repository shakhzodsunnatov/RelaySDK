...
extension NatsClient: NatsConnection {
    ...
    open func connect() throws {
        LoggerManager.shared.logger.debug("Try to connect.")
        guard self.state != .connected else {
            LoggerManager.shared.logger.info("Already connected, skip connection.")
            return
        }
        ...
        if let error = self.connectionError {
            LoggerManager.shared.logger.error("Error while connectig.")
            throw error
        }

        if self.server?.authRequired == true {
            LoggerManager.shared.logger.warning("Authorisation is required.")
    ...
    open func disconnect() {
        LoggerManager.shared.logger.debug("Try to disconnect.")
        do {
            try self.channel?.close().wait()
        } catch {
            LoggerManager.shared.logger.error("\(error.localizedDescription)")
        }
        do {
    ...
    fileprivate func openStream(to url: String) throws {
        ...
        futureConnection.whenFailure({ err in
            LoggerManager.shared.logger.error("\(err.localizedDescription)")
        })
        futureConnection.whenSuccess({channel in
            if hasErrored {
                //throw NatsConnectionError("Server returned an error while trying to connect")
                LoggerManager.shared.logger.error("Server returned an error while trying to connect")
            } else {
        ...
        while isInformed != true {
            ...
            if waitingInterval >= timeout {
                LoggerManager.shared.logger.error("Timeout waitung for information.")
