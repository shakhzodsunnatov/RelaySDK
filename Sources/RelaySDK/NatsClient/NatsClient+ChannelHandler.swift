...
extension NatsClient: ChannelInboundHandler {
    ...
    public func channelActive(context: ChannelHandlerContext) {
        LoggerManager.shared.logger.debug("Channel gets active")
    ...
    public func channelReadComplete(context: ChannelHandlerContext) {
        LoggerManager.shared.logger.debug("Channel read complete")

        guard let chunkLength = inputBuffer?.readableBytes else {
            LoggerManager.shared.logger.warning("Input buffer has no data")
            return
        }

        guard let inputChunk = inputBuffer?.readString(length: chunkLength) else {
            LoggerManager.shared.logger.warning("Input buffer can not read into string")
            return
        }
    ...
    public func channelInactive(context: ChannelHandlerContext) {
        LoggerManager.shared.logger.debug("Channel gets inactive")
    ...
    public func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        LoggerManager.shared.logger.debug("Channel read")
        var byteBuffer = self.unwrapInboundIn(data)
        inputBuffer?.writeBuffer(&byteBuffer)
    }

    public func errorCaught(context: ChannelHandlerContext, error: Error) {
        LoggerManager.shared.logger.error("Error caught: \(error.localizedDescription)")
